const router = require('express').Router();
const productService = require('../services/productService');
const accessoryService = require('../services/accessoryService');
const validate = require('../middlewares/validator');

//MIDDLEWARES
const isGuest = require('../middlewares/isGuest');

//GET
router.get('/', (req, res) => {
    if (Object.values(req.query).length != 0) {
        productService.getSome(req.query)
            .then(cubes => res.render('home', { layout: 'main', title: 'Cubicle Home', cubes }))
            .catch(() => res.status(500).end());

    } else {
        productService.getAll()
            .then(cubes => res.render('home', { layout: 'main', title: 'Cubicle Home', cubes }))
            .catch(() => res.status(500).end());
    }

});

router.get('/about', (req, res) => {
    res.render('about', { layout: 'main', title: 'Cubicle About' });
});

router.get('/create', isGuest(), (req, res) => {
    res.render('create', { layout: 'main', title: 'Cubicle Add' });
});

router.get('/details/:id', (req, res) => {
    productService.getOneWithAccessories(req.params.id)
        .then(cube => {
            let accessories = cube.accessories;
            let isOwn = cube.creator == res.locals.user._id;
            res.render('details', { layout: 'main', title: 'Cubicle Details', cube, accessories, isOwn });
        })
        .catch(() => res.status(500).end());
});

router.get('/details/:id/attach', isGuest(), (req, res) => {
    productService.getOne(req.params.id)
        .then(cube => {
            if (req.user._id != cube.creator) {
                res.redirect('/');
            } else {
                accessoryService.getAllNotin(cube.accessories)
                    .then(accessories => {
                        res.render('attachAccessory', { layout: 'main', title: 'Cubicle Attach Accessory', cube, accessories });
                    })
                    .catch(() => res.status(500).end());
            }
        })
        .catch(() => res.status(500).end());

    accessoryService.getAll()
        .then(accessories => {
            productService.getOne(req.params.id)
                .then(cube => {
                    res.render('attachAccessory', { layout: 'main', title: 'Cubicle Attach Accessory', cube, accessories });
                })
                .catch(() => res.status(500).end());
        })
        .catch(() => res.status(500).end());
});

router.get('/details/:id/edit', (req, res) => {
    productService.getOne(req.params.id)
        .then(cube => {
            if(req.user._id != cube.creator) {
                res.redirect('/');
            } else {
                res.render('editCubePage', { layout: 'main', title: 'Cubicle Edit', cube });
            }
        })
        .catch(() => res.status(500).end());
});

router.get('/details/:id/delete', (req, res) => {
    productService.getOne(req.params.id)
        .then(cube => {
            if(req.user._id != cube.creator) {
                res.redirect('/');
            } else {
                res.render('deleteCubePage', { layout: 'main', cube })
            }
        })
        .catch(err => console.log(err));
});

//POST
router.post('/create', (req, res, next) => {
    let errors, notifications;

    if (req.messages) {
        errors = req.messages.errors;
        notifications = req.messages.notifications;

        if (errors && errors.length > 0) {
            res.render('create', { layout: 'main', title: 'Cubicle Add', errors, name: req.body.name, description: req.body.description, imageUrl: req.body.imageUrl });
            return;
        }

        if (notifications && notifications.length > 0) {
            res.render('create', { layout: 'main', title: 'Cubicle Add', notifications });
            return;
        }
    }

    productService.create(req.body, req.user._id)
        .then(() => res.redirect('/'))
        // .catch(() => res.status(500).end());
        .catch(next);
});

router.post('/details/:id/attach', (req, res) => {
    productService.attachAccessory(req.params.id, req.body.accessory)
        .then(() => {
            res.redirect(`/details/${req.params.id}`);
        })
        .catch(() => res.status(500).end());
});

router.post('/details/:id/edit', (req, res) => {
    productService.updateOne(req.params.id, req.body)
        .then(resp => {
            if (resp.ok == 1) {
                res.redirect('/details/' + req.params.id);
            }
        })
        .catch(() => res.status(500).end());
});

router.post('/details/:id/delete', (req, res) => {
    productService.deleteOne(req.params.id)
        .then(resp => {
            if(resp.ok == 1) {
                res.redirect('/');
            }
        })
        .catch(err => console.log(err));
});

module.exports = router;